    ...
    worker.on('message', function ({ type, data, name }) {
        ...
        switch (type) {
            case 'START_READING_DATA':
                writeStreams[name] = fs.createWriteStream(path.resolve(`out/${fileName}`));
                break;
...
async function readOneFile(url, pipe) {
    const chunkSize = 500;
    const transformer = new stream.Transform({ objectMode: true, highWaterMark: chunkSize });
    ...
    process.on('message', async function ({ type, data }) {
        if (type === 'PARSE_FILES') {
            data.forEach(url => {
                ...
                readOneFile(url, writableStream);
            });
        }
